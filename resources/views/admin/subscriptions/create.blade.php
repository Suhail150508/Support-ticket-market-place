...
@section('page-title', __('Create Subscription Plan'))

@section('content')
<div class="page-header mb-4">
    <div>
        <h1 class="mb-2"><i class="fas fa-plus-circle me-2"></i>{{ __('Create Subscription Plan') }}</h1>
        <p class="mb-0 opacity-75">{{ __('Add a new subscription plan') }}</p>
    </div>
</div>
    ...
    <div class="card-body">
        <form action="{{ route('admin.subscriptions.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="features_text" rows="5" placeholder="{{ __('Live Chat Access') }}&#10;{{ __('Priority Support') }}&#10;{{ __('Unlimited Messages') }}"></textarea>
                <small class="text-muted">{{ __('Enter one feature per line. These will be converted to an array.') }}</small>
            </div>
            <div class="row mb-3">
                <div class="col-md-6">
            ...
            <div class="d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-2"></i>{{ __('Create Plan') }}
                </button>
            </div>
        </form>
    </div>
